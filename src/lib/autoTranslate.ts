...
const pattern = new RegExp(
  `(^|[^\\p{L}\\p{N}])(${entries.map(([en]) => escapeRe(en)).join("|")})(?=$|[^\\p{L}\\p{N}])`,
  "giu",
);

const lookup = new Map(entries.map(([en, ko]) => [en.toLowerCase(), ko]));

export function translateText(input: string): string {
  return input.replace(pattern, (match, lead: string, term: string) => {
...
function walk(root: Node, apply: (node: Text) => void) {
